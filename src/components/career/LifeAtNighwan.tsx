    ...
    <div className="bg-white text-textmain overflow-hidden font-sans">
      ...
      <section className="relative h-[90vh] flex items-center justify-center text-white">
        ...
        <div className="relative z-10 text-center px-6 max-w-5xl container-custom">
          <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-brandOrange font-bold tracking-[0.3em] uppercase text-sm mb-4 block">The Nighwan Ecosystem</motion.span>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="heading-xl !text-white !mb-8">Architecting Vibe. <br /><span className="text-gray-300">Engineering Joy.</span></motion.h1>
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-muted !text-gray-300 mb-10">Culture at Nighwan Tech isn't a perk; it's a strategic imperative. We eliminate corporate friction and synchronize professional growth with human well-being to create a high-velocity value stream of innovation.</motion.p>
          <div className="flex justify-center gap-6 flex-wrap">
            <button onClick={() => openModal("Culture Video")} className="btn-primary">Experience the Culture</button>
            <button onClick={() => openModal("Team Inquiry")} className="btn-secondary !border-white !text-white hover:!bg-white hover:!text-black">Connect with Team</button>
          </div>
        </div>
      </section>

      {/* --- 2. CULTURE PILLARS (Lean Style) --- */}
      <section className="section-padding bg-white">
        <div className="container-custom">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              <h2 className="heading-xl !text-left !mb-6">Operational <br />Transparency</h2>
              <p className="text-brandGreen font-medium text-lg mb-8 leading-relaxed">Most companies suffer from "Hierarchy Stagnation." At Nighwan, we utilize a flat architecture where communication flow is optimized for speed and creative radicalism is the standard.</p>
            ...
            <div className="relative group">
              ...
              <div className="bg-white p-10 rounded-2xl shadow-2xl border border-gray-100">
                <h3 className="text-2xl font-bold mb-6">Internal Vibe Audit</h3>
                <p className="text-gray-600 mb-8 text-sm leading-relaxed">We audit our culture every quarter using <strong>Happiness NPS (Net Promoter Score)</strong> to ensure our environment remains a benchmark for industry standards.</p>
...
[Image of organizational culture model]
                ...
                <button onClick={() => openModal("Gallery")} className="mt-10 text-brandOrange font-bold flex items-center gap-2 hover:gap-4 transition-all">View Cultural Gallery <ArrowRight size={18} /></button>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* --- 3. VIBE STATS (Lean Stats Style) --- */}
      <section className="section-padding bg-textmain text-white">
        <div className="container-custom">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl md:text-4xl font-bold mb-4">Cultural Throughput</h2>
            <p className="text-gray-400">Our engagement metrics outpace industry averages, reflecting a truly sustainable workspace.</p>
          </div>
            ...
            <div key={index} className="group h-[450px] [perspective:1000px]">
              <div className="relative h-full w-full rounded-3xl transition-all duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)] shadow-lg">
                <div className="absolute inset-0 h-full w-full rounded-3xl bg-white p-10 [backface-visibility:hidden] border border-gray-100 flex flex-col items-center text-center justify-center">
                  ...
                  <h3 className="font-bold text-2xl mb-4 text-textmain">{item.t}</h3>
                  <p className="text-gray-500 text-sm leading-relaxed">{item.d}</p>
                  <div className="mt-8 text-brandOrange font-bold text-xs uppercase tracking-widest flex items-center gap-2">Discover More <ArrowRight size={14} /></div>
                </div>
                <div className="absolute inset-0 h-full w-full rounded-3xl bg-textmain p-10 text-white [transform:rotateY(180deg)] [backface-visibility:hidden] flex flex-col justify-center">
                  <h3 className="text-brandOrange font-bold text-xl mb-6">{item.backTitle}</h3>
                  <ul className="space-y-4 mb-8">
                    {item.points.map((p, i) => <li key={i} className="flex items-center gap-3 text-sm font-light text-gray-200"><CheckCircle size={16} className="text-brandOrange" />{p}</li>)}
                  </ul>
                  <button onClick={() => openModal(item.t)} className="bg-white/10 hover:bg-brandOrange text-white text-xs font-bold py-3 px-6 rounded-xl transition-colors border border-white/20">Learn More</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="container-custom">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div className="max-w-xl">
              <h2 className="heading-xl !text-left !mb-4">Standard Perks</h2>
            ...
            <div className="text-gray-400 text-sm font-mono tracking-tighter">/ LIFESTYLE ASSETS</div>
          </div>
          ...
          <div className="bg-white rounded-[3rem] p-8 md:p-16 shadow-inner border border-gray-100">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div className="space-y-8">
                <h2 className="text-3xl md:text-4xl font-bold text-textmain leading-tight">Our Balance Ethos: <br /><span className="text-brandOrange">The 5 Vibe Pillars</span></h2>
                <div className="space-y-6">
                  {[
              ...
              <div className="relative aspect-square md:aspect-auto h-full min-h-[400px]">
                <Image src="https://images.unsplash.com/photo-1511632765486-a01980e01a18?q=80&w=2070" alt="Team Joy" fill className="rounded-3xl object-cover grayscale brightness-75" />
                <div className="absolute inset-0 flex items-center justify-center">
                   <div className="bg-brandOrange text-white p-6 rounded-2xl shadow-2xl rotate-3">
                      <p className="text-2xl font-black italic">"Quality of work is a direct reflection of the quality of life."</p>
                      <span className="text-xs mt-2 block opacity-80">— Nighwan Culture Code</span>
                   </div>
                </div>
              </div>
            </div>
      ...
      <section className="section-padding bg-textmain text-white relative">
        ...
        <div className="container-custom text-center relative z-10">
          ...
          <p className="text-muted !text-gray-400 mb-12">Every day spent in a toxic environment is a day of lost creativity. Join a tribe that values your pulse as much as your code.</p>
          <div className="flex justify-center items-center gap-4 flex-col sm:flex-row">
            <button onClick={() => openModal("Apply Now")} className="btn-primary">Become a Nighwanian</button>
          </div>
        </div>
      </section>
