        ...
        <div className="min-h-screen w-full flex bg-[#f8fafc] font-sans relative overflow-hidden">
            ...
            <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-6 sm:p-12 z-20">
                ...
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-8">
                    <img src="/images/nighlogo-Bxm7gxow.svg" alt="Nighwan Technology" className="h-10 sm:h-12 w-auto object-contain" />
                </motion.div>
                    ...
                    <div className="rounded-2xl bg-white border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden relative">
                        ...
                        <div className="px-8 py-10">
                            ...
                            <div className="mb-8 text-center">
                                ...
                                {mode === 'forgot' && (
                                    <p className="text-sm text-slate-500 mt-2">Enter your registered email address and we'll send you a link to reset your password.</p>
                                )}
                            </div>
                            ...
                            <AnimatePresence mode="wait">
                                {mode === 'login' && redirecting ? (
                                    <motion.div key="redirecting" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs px-4 py-3 rounded-xl mb-6">
                                        <CheckCircle2 size={16} className="shrink-0 text-emerald-500" />
                                        <span>Login successful! Redirecting...</span>
                                    </motion.div>
                                ) : mode === 'login' && state?.error ? (
                                    <motion.div key="login-error" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-start gap-3 bg-red-50 border border-red-200 text-red-600 text-xs px-4 py-3 rounded-xl mb-6" role="alert">
                                        <AlertCircle size={16} className="shrink-0 mt-0.5 text-red-500" />
                                        <span>{state.error}</span>
                                    </motion.div>
                                ) : mode === 'forgot' && forgotState?.error ? (
                                    <motion.div key="forgot-error" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-start gap-3 bg-red-50 border border-red-200 text-red-600 text-xs px-4 py-3 rounded-xl mb-6" role="alert">
                                        <AlertCircle size={16} className="shrink-0 mt-0.5 text-red-500" />
                                        <span>{forgotState.error}</span>
                                    </motion.div>
                                ) : mode === 'forgot' && forgotState?.success ? (
                                    <motion.div key="forgot-success" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="flex items-start gap-3 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs px-4 py-3 rounded-xl mb-6" role="alert">
                                ...
                                <form action={formAction} className="space-y-5">
                                    <div className="flex flex-col gap-1.5">
                                        <div className="relative group">
                                            ...
                                            <input id="email" name="email" type="email" autoComplete="email" required placeholder="admin@example.com" className="w-full pl-11 pr-4 py-3.5 rounded-xl text-sm bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-500/10 transition-all duration-300" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <div className="relative group">
                                            ...
                                            <input id="password" name="password" type={showPass ? "text" : "password"} autoComplete="current-password" required placeholder="••••••••" className="w-full pl-11 pr-11 py-3.5 rounded-xl text-sm bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-500/10 transition-all duration-300" />
                                            <button type="button" onClick={() => setShowPass((v) => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-orange-500 transition-colors">
                                    ...
                                    <div className="flex justify-between items-center px-1">
                                        ...
                                        <button type="button" onClick={() => setMode('forgot')} className="text-xs text-slate-500 hover:text-orange-500 font-medium transition-colors">
                                            Forgot Password?
                                        </button>
                                    </div>
                                ...
                                <form action={forgotFormAction} className="space-y-5">
                                    <div className="flex flex-col gap-1.5">
                                        <div className="relative group">
                                            ...
                                            <input id="forgot-email" name="email" type="email" required placeholder="Enter your email" className="w-full pl-11 pr-4 py-3.5 rounded-xl text-sm bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 focus:bg-white focus:outline-none focus:border-orange-400 focus:ring-4 focus:ring-orange-500/10 transition-all duration-300" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-center mt-4">
                                        <button type="button" onClick={() => setMode('login')} className="flex items-center gap-2 text-sm text-slate-500 hover:text-orange-500 font-medium transition-colors">
                                            <ArrowLeft size={16} />
                                            Back to Login
                                        </button>
                                    </div>
